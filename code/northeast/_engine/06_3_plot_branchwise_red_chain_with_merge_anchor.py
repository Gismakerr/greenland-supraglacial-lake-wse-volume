...
- Output plots directly under "有噪声" and "无噪声" (no extra subfolders).
...
# 当前流程只处理 D 版 PIXC；如需对比 C/D，可在这里放开。
PIXC_VERSIONS = ["D"]
MAX_LAKES_TO_PROCESS = 0
PROCESS_ONLY_LAKE_IDS = []
PROCESS_ONLY_LAKE_BRANCHES = []
ORIGINAL_GROUP_FOLDER_NAME = "原版"
RANKED_GROUP_FOLDER_NAME = "重新分组版"

DISABLE_DEGRADED_POINTS_IN_CROSSTRACK_BAND = True
# 初始红组覆盖天数少于该值时，不直接认为是稳定主链。
INITIAL_RED_GROUP_MIN_DAYS = 3
# 红链两端的短灰链长度上限；在该范围内会启用更严格的边缘去噪。
EDGE_GRAY_SHORT_CHAIN_MAX_LEN = 2
# 最终保存 branch 结果前，至少需要多少个红点。
RED_POINTS_MIN_TO_SAVE = 5
RED_DAYS_LT5_FOLDER_NAME = "red_days_lt_5"
EXCLUDE_FOLDER_NAME = "exclude"
LOCAL_EXTREMA_NOISE_THRESHOLD_M = 0.5
# 若红点平均不确定性 / 水位幅度过高，则整条 branch 视作高噪声。
HIGH_RATIO_NOISE_THRESHOLD = 0.3
HIGH_RATIO_NOISE_FOLDER_NAME = "high_ratio_noise_gt_0p3"
# Force selected lakes to be treated as noise-only outputs (not exported to no_noise).
FORCE_NO_NOISE_LAKE_IDS = {164, 218}
# Force selected branches to stay out of no_noise after reruns.
FORCE_NO_NOISE_BRANCHES = {
    (4, 1),
    (16, 2),
    (22, 1),
    (26, 2),
...
PLOT_X_GLOBAL_START = pd.Timestamp("2024-06-13")
PLOT_X_TICK_START = pd.Timestamp("2024-06-13")
...
DUAL_AREA_FOLDER_NAME = "无噪声水位面积双轴图"
RED_WSE_RECORDS_FOLDER_NAME = "去噪后红点水位记录"
FILTERED_LAKE_SHP_PATH = SCRIPT_DIR.parent / "result" / "02_Lake_Extraction" / "04_Filtered_Lakes" / "Water_Max_Filtered_Polygons.shp"
MAP_MAIN_DIR = SCRIPT_DIR.parent / "result" / "画图" / "data" / "main"
AREA_COLOR = "#2F6DF6"
AREA_LINESTYLE = "-"
AREA_LINEWIDTH = 1.5
AREA_LINE_ALPHA = 0.9
...
def load_base_module():
    # 复用 06_2 的基础红链/去噪能力，并重定向输入输出路径到当前工程目录。
    ...
    output_root_dir = str(SCRIPT_DIR.parent / "result")
...
def drop_degraded_points_in_cross_track_band(raw_wse_df):
    # 先删除跨轨质量较差的 relaxed_no_cross_track 点，避免后续误判。
...
def keep_one_wse_per_scene(raw_wse_df):
    ...
    work = work.sort_values(
        ["lake_id", "branch_index", "date", "source_layer", "_type_rank_sort", "_wse_std_sort", "_wse_sort"]
    ).copy()
    before = len(work)
    work = work.drop_duplicates(subset=["lake_id", "branch_index", "date", "source_layer"], keep="first").copy()
    after = len(work)
    if after < before:
        print(f"[06_3] one-per-scene dedupe: {before} -> {after}", flush=True)
    return work.drop(columns=["_type_rank_sort", "_wse_std_sort", "_wse_sort"], errors="ignore")
...
def render_branch_plot(base_module, lake_id, branch_index, records, segments, out_path, show_noise, fixed_xlim=None, display_lake_id=None):
    # show_noise=True 输出带噪声版本；False 输出去噪后的主链版本。
...
def main():
    # 06_3 总入口：读取 06_2/06_1 结果，按 branch 做增强去噪并输出图表。
    base = load_base_module()
    filtered_lakes_gdf = load_filtered_lake_polygons()

    vc_d_both_name = "CD_Both"
    vc_d_c_only_name = "C_Only"
    vc_d_d_only_name = "D_Only"
    with_noise_name = "noise"
    ...
    selected_lake_branches = (
        {(int(lake_id), int(branch_index)) for lake_id, branch_index in PROCESS_ONLY_LAKE_BRANCHES}
        if PROCESS_ONLY_LAKE_BRANCHES
        else set()
    )
    ...
    for method_name in base.METHOD_NAMES:
        ...
        if filtered_lakes_gdf is not None and not filtered_lakes_gdf.empty:
            allowed_ids = set(filtered_lakes_gdf["lake_id"].astype(int).tolist())
            if EXCLUDED_LAKE_IDS:
                allowed_ids |= {int(x) for x in EXCLUDED_LAKE_IDS}
            lake_summary_df = lake_summary_df[
                pd.to_numeric(lake_summary_df["lake_id"], errors="coerce").astype("Int64").isin(sorted(allowed_ids))
            ].copy()
        ...
        if lake_summary_df.empty:
            ...
            continue
        excluded_lake_summary_df = pd.DataFrame(columns=lake_summary_df.columns)
        if EXCLUDED_LAKE_IDS:
            excluded_lake_summary_df = lake_summary_df[
        ...
        for pixc_version in selected_versions:
            ...
            exclude_dir = os.path.join(version_root_dir, EXCLUDE_FOLDER_NAME)
            version_csv_dir = os.path.join(method_paths["csv_dir"], f"v{pixc_version}")
            high_ratio_rows = []
            # Clear previous run outputs to avoid stale files remaining in no_noise/noise folders.
            for d in [
                with_noise_dir,
                no_noise_dir,
            ...
            for lake_pos, summary_row in enumerate(lake_summary_df.to_dict("records"), start=1):
                ...
                for branch_index in unique_branches:
                    if selected_lake_branches and (int(lake_id), int(branch_index)) not in selected_lake_branches:
                        continue
                    ...
                    if (int(lake_id), int(branch_index)) in EXCLUDED_LAKE_BRANCHES:
                        ...
                        print(
                            f"[06_3] export excluded branch to exclude: lake={int(lake_id)} branch={int(branch_index)}",
                            flush=True,
                        )
                        total_plots += 1
                        continue
                    ...
                    is_forced_noise_lake = int(lake_id) in FORCE_NO_NOISE_LAKE_IDS
                    is_forced_noise_branch = (int(lake_id), int(branch_index)) in FORCE_NO_NOISE_BRANCHES
                    ...
                    if is_forced_noise_lake or is_forced_noise_branch:
                        force_noise_csv = os.path.join(
                            high_ratio_noise_dir,
                            f"lake_{lake_id}_branch_{branch_index}_forced_noise_v{pixc_version}.csv",
                        )
                        red_only_df.to_csv(force_noise_csv, index=False, encoding="utf-8-sig")
                    ...
                    is_noise_only = bool(is_high_ratio_noise or is_forced_noise_lake or is_forced_noise_branch)
                    ...
                    total_plots += 1 if is_noise_only else 3
            ...
            excluded_total_lakes = len(excluded_lake_summary_df)
            for excluded_pos, summary_row in enumerate(excluded_lake_summary_df.to_dict("records"), start=1):
                lake_id = int(summary_row["lake_id"])
                lake_df = version_df[version_df["lake_id"] == lake_id].copy()
                if lake_df.empty:
                    continue
                ...
                print(
                    f"[06_3] {method_name} v{pixc_version} excluded {excluded_pos}/{excluded_total_lakes} done | lake={lake_id} | branches={len(unique_branches)}",
                    flush=True,
                )
            if high_ratio_rows:
                high_ratio_df = pd.DataFrame(high_ratio_rows).sort_values(["ratio", "lake_id", "branch_index"], ascending=[False, True, True])
