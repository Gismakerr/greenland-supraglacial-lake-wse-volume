...
def load_features_df(csv_path):
    ...
    # Unified size rule for current project:
    # area_km2 >= 0.0625 -> Large; otherwise Small.
    work["size_category"] = np.where(
        pd.to_numeric(work["area_km2"], errors="coerce") >= 0.0625,
        "Large",
        "Small",
    )
    return work
...
def apply_common_filters(df):
    if df.empty:
        return df
    work = df.copy()
    if os.path.exists(LAKE_FILTER_SHP):
        shp = gpd.read_file(LAKE_FILTER_SHP)
        if "lake_id" in shp.columns:
            allowed = set(pd.to_numeric(shp["lake_id"], errors="coerce").dropna().astype(int).tolist())
            work = work[pd.to_numeric(work["lake_id"], errors="coerce").astype("Int64").isin(sorted(allowed))].copy()
...
def classify_final_curve_size_category(max_area_km2):
    ...
    if pd.isna(value):
        return "Small"
    return "Large" if float(value) >= 0.0625 else "Small"
