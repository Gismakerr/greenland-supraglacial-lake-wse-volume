...
LAKES_BUFFERED_SHP = os.path.join(OUTPUT_ROOT_DIR, "02_Lake_Extraction", "04_Filtered_Lakes", "Water_Max_Filtered_Polygons_Buffered.shp")
...
LAKE_FILTER_SHP = os.path.join(OUTPUT_ROOT_DIR, "02_Lake_Extraction", "04_Filtered_Lakes", "Water_Max_Filtered_Polygons.shp")
...
AUDIT_COLUMNS = [
    ...
    "cross_track_mode", "cross_track_relaxed",
]
...
def resolve_lake_date_window(summary_row):
    ...
    if pd.notna(window_start):
        window_start = window_start - pd.Timedelta(days=DATE_PAD_BEFORE_DAYS)
    if pd.notna(window_end):
        window_end = window_end + pd.Timedelta(days=DATE_PAD_AFTER_DAYS)
...
def resolve_water_period_window(summary_row):
    return (
        to_datetime_ymd(summary_row.get("water_period_start", "")),
        to_datetime_ymd(summary_row.get("water_period_end", "")),
    )
...
def load_allowed_lake_ids_from_shp(shp_path):
    ...
    if gdf.empty or "lake_id" not in gdf.columns:
        return set()
    vals = pd.to_numeric(gdf["lake_id"], errors="coerce").dropna().astype(int).tolist()
    return set(vals)
...
def list_swot_layers_by_day(swot_gdb_path):
    if not os.path.exists(swot_gdb_path):
        return {}
    try:
        layer_names = fiona.listlayers(swot_gdb_path)
    except Exception:
        return {}
    by_day = defaultdict(list)
...
def build_mask_specs(summary_row, branch_df, branch_start_df, topology_features_gdf, lake_id):
    period_start = to_datetime_ymd(summary_row.get("water_period_start", ""))
    period_end = to_datetime_ymd(summary_row.get("water_period_end", ""))
    specs = []
...
def _sanitize_polygon_gdf(gdf):
    """Prepare polygon-only geometries for stable GDB export."""
    if gdf is None or gdf.empty:
        return gpd.GeoDataFrame(columns=getattr(gdf, "columns", []), geometry="geometry", crs=getattr(gdf, "crs", None))
    out = gdf.copy()
    ...
    try:
        ...
        pass
    geom_type = out.geometry.geom_type.astype(str)
    out = out[geom_type.isin(["Polygon", "MultiPolygon"])].copy()
    if out.empty:
        return gpd.GeoDataFrame(columns=gdf.columns, geometry="geometry", crs=gdf.crs)
    ...
    for method_name in METHOD_NAMES:
        ...
        if allowed_lake_ids:
            scoped_summary = scoped_summary[
                pd.to_numeric(scoped_summary["lake_id"], errors="coerce").astype("Int64").isin(sorted(allowed_lake_ids))
            ].copy()
        if TARGET_LAKE_IDS:
        ...
        if all_mask_gdfs:
            ...
            mask_out = _sanitize_polygon_gdf(mask_out)
            if mask_out.empty:
                progress(f"[06_1] {method_name}: mask gdf empty after geometry sanitize, skip mask GDB export")
            else:
        ...
        if all_raw_wse_gdfs:
            ...
            raw_wse_out = _sanitize_polygon_gdf(raw_wse_out)
            if raw_wse_out.empty:
                progress(f"[06_1] {method_name}: raw_wse gdf empty after geometry sanitize, skip raw_wse GDB export")
            else:
