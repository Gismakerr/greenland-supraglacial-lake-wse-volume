...
S2_IMAGE_DIR = r"X:\2024_西南水位曲线\data1\S2_22WEV_2024-06-01_to_2024-08-30"
...
def threshold_otsu(values: np.ndarray) -> float:
...
def parse_preview_scene_info(filename):
    match = re.search(r"overlay_(\d+)_(\d{8})_(\d{6})_(S2A|S2B|S2X)\.png$", filename, re.IGNORECASE)
    if not match:
        return None
    return {
...
def find_scene_file(directory, date_str, time_str, satellite, suffix):
    ...
    if len(candidates) == 1:
        return candidates[0]
    # fallback for unknown satellite label in preview name (e.g., S2X)
    fallback_candidates = glob.glob(os.path.join(directory, f"*{date_str}*.{suffix}"))
    fallback_exact = [path for path in fallback_candidates if exact_token in os.path.basename(path)]
    if len(fallback_exact) == 1:
        return fallback_exact[0]
    if len(fallback_exact) > 1:
        return sorted(fallback_exact)[0]
...
def get_lake_tasks():
    ...
    if TARGET_LAKE_POSITIONS:
        try:
            ...
            if "lake_id" in buffer_gdf.columns:
                if "ice_flag" in buffer_gdf.columns:
                    allowed_lake_ids = set(
                        int(row["lake_id"])
                        for _, row in buffer_gdf[buffer_gdf["ice_flag"].astype(int) == 1].iterrows()
                    )
                elif "lake_pos" in buffer_gdf.columns:
                    allowed_lake_ids = set(
                        int(row["lake_id"])
...
def init_worker(batch_tasks):
    ...
    if TARGET_SIZE_CATEGORIES and "lake_type" in buffer_gdf.columns:
        buffer_gdf = buffer_gdf[buffer_gdf["lake_type"].isin(TARGET_SIZE_CATEGORIES)].copy()
    if TARGET_SIZE_CATEGORIES and "lake_type" in area_gdf.columns:
        area_gdf = area_gdf[area_gdf["lake_type"].isin(TARGET_SIZE_CATEGORIES)].copy()
...
def process_single_lake(lake_id, method_cfg, method_paths):
    ...
    for scene_info in preview_scene_infos:
        ...
        rgb_path = find_scene_file(S2_IMAGE_DIR, date, time_str, satellite, "tif")
