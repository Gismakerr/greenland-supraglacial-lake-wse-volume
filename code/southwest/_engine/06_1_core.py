...
FIXED_WINDOW_START = "20240601"
FIXED_WINDOW_END = "20240815"
APPLY_LAKE_FILTER_FROM_SHP = False
...
_env_pixc_dir_d = str(os.environ.get("PIXC_DIR_D", "")).strip()
if _env_pixc_dir_d:
    PIXC_DIR_D = _env_pixc_dir_d

_env_target_lake_ids = _parse_env_int_list("TARGET_LAKE_IDS")
if _env_target_lake_ids:
    TARGET_LAKE_IDS = sorted(set(_env_target_lake_ids))
else:
    TARGET_LAKE_IDS = []

_env_disable_resume = str(os.environ.get("DISABLE_06_1_RESUME", "")).strip().lower()
if _env_disable_resume in {"1", "true", "yes", "y"}:
    RESUME_FROM_INDEX = False
    RESUME_SKIP_PRECHECK_WHEN_INDEX_EXISTS = False
    LAKE_RESUME_FROM_SUMMARY = False
...
def build_lake_windows(topology_df):
    ...
    for lake_id, grp in topology_df.groupby("lake_id", sort=True):
        ...
        rows.append(
            {
                "lake_id": int(lake_id),
                "water_period_start": str(FIXED_WINDOW_START),
                "water_period_end": str(FIXED_WINDOW_END),
...
def expand_lake_windows_to_all_lakes(lake_windows_df, lake_geom_map, topology_df=None):
    ...
    start_text = str(FIXED_WINDOW_START)
    end_text = str(FIXED_WINDOW_END)
...
def main():
    ...
    for method_name in METHOD_NAMES:
        ...
        allowed_lake_ids = load_allowed_lake_ids_from_shp(LAKE_FILTER_SHP) if APPLY_LAKE_FILTER_FROM_SHP else set()
        if allowed_lake_ids:
            before_count = int(len(lake_windows_df))
            lake_windows_df = lake_windows_df[
        ...
        if TARGET_LAKE_IDS:
            before_count = int(len(lake_windows_df))
            lake_windows_df = lake_windows_df[
                pd.to_numeric(lake_windows_df["lake_id"], errors="coerce").astype("Int64").isin(sorted(TARGET_LAKE_IDS))
            ].copy()
            progress(
                f"[06_1] {method_name}: apply TARGET_LAKE_IDS filter "
                f"{before_count}->{len(lake_windows_df)}"
            )
